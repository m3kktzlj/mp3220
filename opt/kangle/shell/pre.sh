...
if [ "$OS6" = "$centos6" ];then
	...
	rpm -ivh $mrocdn_2231/etc/yum.repos.d/mysql-community-release-el6-7.noarch.rpm --nodeps --force
	if [ "$mysql_ver" = "7" ]; then
	...
	rpm -ivh $mrocdn_2231/etc/yum.repos.d/mysql-community-release-el7-7.noarch.rpm --nodeps --force
	if [ "$mysql_ver" = "7" ]; then
		wget -q $mrocdn_2231/etc/yum.repos.d/mysql-community-7.repo -O /etc/yum.repos.d/mysql-community.repo
	elif [ "$mysql_ver" = "8" ]; then
		wget -q $mrocdn_2231/etc/yum.repos.d/mysql-community-8.repo -O /etc/yum.repos.d/mysql-community.repo
...
————————————————————————————————————————————————————"
wget -q $mrocdn_2231/opt/kangle/shell/install.sh -O install.sh;sh install.sh $kangle_ver $kangle_completed $select_ver $mysql_ver $mysql_root_password
else
clear
echo -e "
————————————————————————————————————————————————————
