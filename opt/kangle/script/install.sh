...
function installkangle(){
	cd ${dir01}
	wget -q ${mpcdn_2220}/opt/kangle/script/kangle.sh -O kangle.sh;sh kangle.sh $kangle_ver $kangle_completed | tee ${dir02}/kangle.log
}
function installep(){
	cd ${dir01}
	wget -q ${mpcdn_2220}/opt/kangle/script/easypanel.sh -O easypanel.sh;sh easypanel.sh | tee ${dir02}/easypanel.log
}
function installftp(){
	cd ${dir01}
	wget -q ${mpcdn_2220}/opt/kangle/script/pureftp.sh -O pureftp.sh;sh pureftp.sh | tee ${dir02}/pureftp.log
}
function installsql(){
	cd ${dir01}
	wget -q ${mpcdn_2220}/opt/kangle/script/mysql_install.sh -O mysql_install.sh;sh mysql_install.sh $mysql_ver $mysql_root_password | tee ${dir02}/mysql_install.log
}
function installpm(){
	cd ${dir01}
...
function install_php_compile(){
	cd ${dir01}
	wget -q ${mpcdn_2220}/opt/kangle/script/php_compile.sh -O php_compile.sh | tee ${dir02}/php_compile-00.log
...
function installend(){
	wget -q ${mpcdn_2220}/opt/kangle/script/complete.sh -O complete.sh;sh complete.sh $mysql_root_password | tee ${dir02}/complete.log
	exit 0
}
stat_iptables
