...
if [ "$mysql_ver" = "8" ]; then
	wget -q $runner_url/opt/qiangidc/config/mysql8.0/my-01.cnf -O /etc/my.cnf
	if [ "$release" = "8" ]; then
		mkdir /home/mysql
		chown -R mysql:mysql /home/mysql
	fi
elif [ "$mysql_ver" = "7" ]; then
	wget -q $runner_url/opt/qiangidc/config/mysql5.7/my-01.cnf -O /etc/my.cnf
else
	wget -q $runner_url/opt/qiangidc/config/mysql5.6/my-01.cnf -O /etc/my.cnf
fi
...
#重置MySQL密码
wget -q $runner_url/opt/qiangidc/shell/mysql_password.sh -O mysql_password.sh;sh mysql_password.sh $mysql_root_password
