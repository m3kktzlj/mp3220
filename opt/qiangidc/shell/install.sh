...
function installkangle(){
	cd $dir03
	wget -q $runner_url/opt/qiangidc/shell/kangle.sh -O kangle.sh;sh kangle.sh $kangle_ver $kangle_completed | tee $dir04/kangle.log
}
function installep(){
	cd $dir03
	wget -q $runner_url/opt/qiangidc/shell/easypanel.sh -O easypanel.sh;sh easypanel.sh | tee $dir04/easypanel.log
}
function installftp(){
	cd $dir03
	wget -q $runner_url/opt/qiangidc/shell/pureftp.sh -O pureftp.sh;sh pureftp.sh | tee $dir04/pureftp.log
}
function installsql(){
	cd $dir03
	wget -q $runner_url/opt/qiangidc/shell/mysql.sh -O mysql.sh;sh mysql.sh $mysql_ver $mysql_root_password | tee $dir04/mysql.log
}
function installpm(){
	cd $dir03
...
function install_php_compile(){
	cd $dir03
	wget -q $runner_url/opt/qiangidc/shell/php_compile.sh -O php_compile.sh | tee $dir04/php_compile-00.log
...
function install_php_rapidly(){
	cd $dir03
	wget -q $runner_url/opt/qiangidc/shell/php_rapidly.sh -O php_rapidly.sh
...
function installend(){
	wget -q $runner_url/opt/qiangidc/shell/complete.sh -O complete.sh;sh complete.sh $mysql_root_password | tee $dir04/complete.log
	exit 0
}
stat_iptables
